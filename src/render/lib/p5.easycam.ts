...
export namespace Dw {
  ...
  export class EasyCam {
    ...
    LOOK: Vec3;
    UP: Vec3;
    AXIS: { YAW: number; PITCH: number; ALL: any; ROLL: number };
    ...
    distance_min: number;
    distance_max: number;
    state: { distance: number; center: Vec3; rotation: Vec4; copy: () => any };
    state_reset: any;
    state_pushed: any;
    viewport: Vec4;
     ...
     */
    constructor(
      renderer: { elt: { getBoundingClientRect: () => any }; width: number; height: number },
      args?: {
        distance: number;
        center: Vec3;
        rotation: Vec4;
      ...
      this.state = {
        ...
        center: args.center.slice() as Vec3, // vec3
        rotation: args.rotation.slice() as Vec4, // quaternion

        copy: function (dst?: { distance: number; center: Vec3; rotation: Vec4 }) {
          return {
      ...
      this.viewport = args.viewport.slice() as Vec4;

      // offset of the canvas in the container
      this.offset = args.offset.slice() as [number, number];
      ...
      this.mouse = {
        ...
        insideViewport(x: number, y: number) {
          let x0 = cam.viewport[0],
            x1 = x0 + cam.viewport[2];
          let y0 = cam.viewport[1],
        ...
        evaluateTouches(event: TouchEvent) {
      ...
      this.dampedZoom = new DampedAction(function (d: number) {
        cam.zoom(d * cam.getZoomMult());
      });
      this.dampedPanX = new DampedAction(function (d: number) {
        cam.panX(d * cam.getPanMult());
      });
      this.dampedPanY = new DampedAction(function (d: number) {
        cam.panY(d * cam.getPanMult());
      });
      this.dampedRotX = new DampedAction(function (d: number) {
        cam.rotateX(d * cam.getRotationMult());
      });
      this.dampedRotY = new DampedAction(function (d: number) {
        cam.rotateY(d * cam.getRotationMult());
      });
      this.dampedRotZ = new DampedAction(function (d: number) {
    ...
    attachListener(
      el: any,
      ev: any,
      fx: { el: { addEventListener: (arg0: any, arg1: any, arg2: any) => void }; ev: any; op: any },
      op: any,
    ) {
      if (!el || el === fx.el) {
        return;
    ...
    detachListener(fx: {
      el: { removeEventListener: (arg0: any, arg1: any, arg2: any) => void } | undefined;
      ev: any;
      op: any;
    }) {
      if (fx.el) {
    ...
    /** */
    setViewport(viewport: Vec4) {
      this.viewport = viewport.slice() as Vec4;
    }
    ...
    rotate(axis: Vec3, angle?: number) {
      let state = this.cam.state;
      if (angle) {
        let new_rotation = Rotation.create({ axis, angle }) as Vec4;
    ...
    setInterpolatedCenter(valA: Vec3, valB: Vec3, t: number) {
      //@ts-ignore
    ...
    setInterpolatedRotation(valA: Vec4, valB: Vec4, t: number) {
      //@ts-ignore
     ...
     *
     */
    setCenter(center: Vec3, duration: number) {
     ...
     *
     */
    setRotation(rotation: Vec4, duration: number) {
     ...
     */
    setState(other: { distance: number; center: Vec3; rotation: Vec4 }, duration: number) {
      if (other) {
    ...
    endHUD(renderer: {
      drawingContext: any;
      uMVMatrix: { set: (arg0: any) => void };
      uPMatrix: { set: (arg0: any) => void };
      pop: (arg0: any) => void;
    }) {
      let cam = this.cam;
      renderer = renderer ?? cam.renderer;
  ...
  export class Interpolation {
     ...
     */
    start(valA: any, valB: any, duration?: number, actions?: DampedAction[]) {
  ...
  export namespace Rotation {
    export function assert(dst?: Vec4): Vec4 {
      return dst ?? [1, 0, 0, 0];
    }

    /** returns an identity rotation [1,0,0,0] */
    export function identity(): Vec4 {
      return [1, 0, 0, 0];
    }
     ...
     */
    export function applyToVec3(rot: Vec4, vec: Vec3, dst?: Vec3) {
      let [x, y, z] = vec;
      let [q0, q1, q2, q3] = rot;
     ...
     */
    export function applyToRotation(rotA: Vec4, rotB: Vec4, dst: Vec4): Vec4 {
     ...
     * @returns  dst - resulting rotation
     */
    export function slerp(rotA: Vec4, rotB: Vec4, t: number, dst: Vec4): Vec4 {
      let [a0, a1, a2, a3] = rotA;
      let [b0, b1, b2, b3] = rotB;
    ...
    export function create(
      ...
      if (def.angles_xyz) {
        ...
        let rotX: Vec4 = [Math.cos(ax), Math.sin(ax), 0, 0];
        let rotY: Vec4 = [Math.cos(ay), 0, Math.sin(ay), 0];
        let rotZ: Vec4 = [Math.cos(az), 0, 0, Math.sin(az)];
  ...
  export namespace Vec3 {
    export function assert(dst?: Vec3) {
      return dst ?? ([0, 0, 0] as unknown as Vec3);
    }
    // test is Array
    export function isScalar(arg: any) {
    ...
    export function add(a: Vec3, b: Vec3 | number, dst: Vec3) {
      dst = Vec3.assert(dst);
      if (!Array.isArray(b)) {
        dst[0] = a[0] + b;
        dst[1] = a[1] + b;
    ...
    export function mult(a: Vec3, b: Vec3 | number, dst: Vec3) {
      dst = Vec3.assert(dst);
      if (!Array.isArray(b)) {
        dst[0] = a[0] * b;
        dst[1] = a[1] * b;
    ...
    /** squared length  */
    export function magSq(a: Vec3) {
      return a[0] * a[0] + a[1] * a[1] + a[2] * a[2];
    }

    /** length  */
    export function mag(a: Vec3) {
    ...
    /** angle  */
    export function angle(v1: Vec3, v2: Vec3) {
      let normProduct = mag(v1) * mag(v2);
      if (normProduct === 0.0) {
      ...
      if (_dot < -threshold || _dot > threshold) {
      ...
      return Math.acos(_dot / normProduct);
    }

    /** linear interpolation: <pre> dst = a * (1 - t) + b * t </pre> */
    export function mix(a: Vec3, b: Vec3, t: number, dst?: Vec3) {
      dst = dst ?? ([0, 0, 0] as unknown as Vec3);
