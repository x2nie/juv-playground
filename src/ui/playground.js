...
class TabbedEditor extends Component {
   setup() {
      ...
      onPatched(() => {
         ...
         if (content === false) {
            const tab =
               this.props.js !== false ? "js" : this.props.xml ? "xml" : "css";
...
export class Playground extends Component {
   ...
   updatePanelHeight({ delta }) {
      let height =
         this.state.topPanelHeight ||
         document.querySelector(".tabbed-editor").clientHeight;
...
async function start() {
   ...
   console.info(
      `This application is using Owl built with the following commit:`,
      commit
   );
